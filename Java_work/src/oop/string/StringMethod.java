...
public class StringMethod {
	...
	public static void main(String[] args) {
		...
		char c= str.charAt(4); //j
		System.out.println("4번인덱스:" +c);
		
		//subString(begin, end)
		// : 문자열을 범위를 지정하여 추출
		String ss1 = str.substring(1, 5); // 1번이상 5번ㅁ ㅣ만
		...
		///length() :문자열의 총길이 반환.
		int len =str.length();
		System.out.println("len : " + len); //공백포함 !
		...
		System.out.println(java.replace("습니", "")); // 지워버림
		...
		int i = Integer.parseInt(s1); // 문자열을 정수 변환
		double d = Double.parseDouble(s2); // 문자열을 실수 변환 
